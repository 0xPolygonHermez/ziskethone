...
// Cross-checks mulmod_short against the independent arith256_mod, and
...
int main() {
    for (int iter = 0; iter < 2000; ++iter) {
        ...
        if (is_zero4(m)) m[0]=1;
        // mulmod_short(a,b,m) == arith256_mod(a,b,0,m)
        uint64_t r1[4], r2[4], zero[4]={0,0,0,0};
        mulmod_short(a,b,m,r1);
        arith256_mod(a,b,zero,m,r2);
        if (!eq4(r1,r2)) { check(false,"mulmod_short == arith256_mod"); break; }
        // mulmod_short(a,a,m) (the square step) == arith256_mod(a,a,0,m)
        mulmod_short(a,a,m,r1);
        arith256_mod(a,a,zero,m,r2);
        if (!eq4(r1,r2)) { check(false,"mulmod_short(square) == arith256_mod"); break; }
    ...
    if (g_fail==0) check(true, "2000 random short-path cases (mulmod, rem) vs references");

    // edge cases
    { uint64_t a[4]={5,0,0,0}, m[4]={5,0,0,0}, r[4]; uint64_t one[4]={1,0,0,0};
      mulmod_short(a,one,m,r); check(is_zero4(r), "5*1 mod 5 == 0"); }
    { uint64_t a[4]={7,0,0,0}, b[4]={1,0,0,0}, m[4]={100,0,0,0}, r[4];
      mulmod_short(a,b,m,r); check(r[0]==7&&r[1]==0, "7*1 mod 100 == 7"); }
